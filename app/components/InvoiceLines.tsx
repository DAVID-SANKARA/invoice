...
const InvoiceLines: React.FC<Props> = ({ invoice, setInvoice }) => {
  ...
  const handleDescriptonChange = (index: number, value:string) =>{
    const updatedLines = [...invoice.lines]
  ...
  return (
    <div className="h-fit bg-base-200 p-5 rounded-x1 w-full">
              ...
              <tr key={line.id}>
                ...
                <td>
                  <input
                    ...
                    className="input input-sm input-bordered w-full"
                    min={0}
                    onChange={(e)=> handleDescriptonChange(index, e.target.value)}
